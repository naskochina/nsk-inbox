#!/usr/bin/env python3
# NSK_IN_buttons_test5055.py
# Инжектира ДВА бутона в Files action меню (#actSheet) на TEST версията (5055):
#   1) "Copy"           — копира съдържанието на файла в clipboard (работи на HTTP)
#   2) "Share to Claude"— copy + native share (ако браузърът позволи на HTTPS)
#
# Таргетира файла, който DEV услугата (nsk-webtool3-dev) реално чете.
# НЕ пипа production (5001). Backup първо, само вмъква, verify накрая.
...
SERVICE = "nsk-webtool3-dev"   # DEV услугата (5055)

print(f"{MAG}{B}=== NSK: Copy + Share бутони в TEST (5055) ==={R}", flush=True)
print(f"{YEL}Таргетира само dev услугата. Production (5001) НЕ се пипа.{R}", flush=True)
time.sleep(0.3)

# ---------- НАМЕРИ ФАЙЛА, КОЙТО DEV УСЛУГАТА ЧЕТЕ ----------
step(f"Намирам файла, който {SERVICE} реално чете ...")
PATH = None
try:
    out = subprocess.run(["systemctl","show",SERVICE,"--property=ExecStart"],
...
if not PATH or not os.path.isfile(PATH):
    err(f"Не намерих dev файла (открих: {PATH}). Спирам.")
    sys.exit(1)
ok(f"Dev файл (5055): {PATH}")
time.sleep(0.2)
...
# ---------- IDEMPOTENCY ----------
already = []
if "actCopyContent" in src: already.append("Copy")
if "actShareClaude" in src: already.append("Share")
if already:
    warn(f"Вече инсталирани: {', '.join(already)}. Ще вмъкна само липсващите.")

# ---------- КОТВИ ----------
delete_anchor = '<div class="act danger" onclick="actDelete()">'
if delete_anchor not in src:
    err("Не намерих Delete бутона (котва за HTML) в #actSheet. Спирам."); sys.exit(1)
fn_anchor = "function actDownload()"
if fn_anchor not in src:
    err("Не намерих 'function actDownload()' (котва за JS). Спирам."); sys.exit(1)
...
html_to_add = ""
if "actCopyContent" not in src: html_to_add += COPY_BTN
if "actShareClaude" not in src: html_to_add += SHARE_BTN

if html_to_add:
    step("Вмъквам HTML бутон(и) преди Delete ...")
    src = src.replace(delete_anchor, html_to_add + delete_anchor, 1)
    ok("HTML бутон(и) вмъкнати")
    time.sleep(0.2)
...
js_to_add = ""
if "function actCopyContent()" not in src: js_to_add += COPY_FN
if "function actShareClaude()" not in src: js_to_add += SHARE_FN

if js_to_add:
    step("Вмъквам JS функция(и) преди actDownload() ...")
    src = src.replace(fn_anchor, js_to_add + fn_anchor, 1)
    ok("JS функция(и) вмъкнати")
    time.sleep(0.2)

if not html_to_add and not js_to_add:
    warn("Нищо за добавяне — двата бутона вече са налице.")
    sys.exit(0)
...
try:
    ...
    err(f"Записът се провали: {e} — върни: cp {bak} {PATH}"); sys.exit(1)
time.sleep(0.2)
...
if r.returncode != 0:
    err(f"restart се провали: {r.stderr}. Върни: cp {bak} {PATH} && systemctl restart {SERVICE}"); sys.exit(1)
ok("restart OK")
time.sleep(1.0)
...
if copy_ok and share_ok and active == "active":
    ok("Copy + Share бутоните са налице, услугата е ACTIVE ✔")
    print(f"{GRN}{B}=== ГОТОВО. Отвори 5055 в Private tab и тествай двата бутона ==={R}", flush=True)
